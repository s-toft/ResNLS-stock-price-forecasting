...
#rs = bs.query_history_k_data("sh.000001", fields, start_date="2012-01-01", end_date="2022-12-31", frequency="d", adjustflag="2") <-- updated since code came out
data_list = []
...
#### ALL THE BELOW IS ADDED BY SIGNE JUST FOR COMPARISON REASONS ##
# Save baseline (leaky) predictions/targets with explicit names
baseline_leaky_preds = predictions.astype(np.float64).flatten()   # already inverse-transformed
baseline_leaky_targets = y.astype(np.float64).flatten()           # true values for baseline

np.save("baseline_leaky_preds.npy", baseline_leaky_preds)
np.save("baseline_leaky_targets.npy", baseline_leaky_targets)

print(f"[Saved] baseline_leaky_preds.npy shape={baseline_leaky_preds.shape}, "
      f"baseline_leaky_targets.npy shape={baseline_leaky_targets.shape}")
...
plt.title("Stock Price Prediction (ResNLS)")
plt.xlabel("Date")
plt.ylabel("Price")
plt.legend()
plt.show()
